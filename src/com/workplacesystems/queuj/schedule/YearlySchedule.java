...
public class YearlySchedule extends Schedule
{
    ...
    private static final long serialVersionUID = YearlySchedule.class.getName().hashCode() + 1;

    private final int interval_years;
    private final int scheduled_month;
    private final int scheduled_day;
    private final int scheduled_hour;
    private final int scheduled_minute;

    /** Creates a new instance of YearlySchedule */
    YearlySchedule(int interval_years, int scheduled_month, int scheduled_day, int scheduled_hour, int scheduled_minute)
    {
        this.interval_years = interval_years;
        this.scheduled_month = scheduled_month;
        this.scheduled_day = scheduled_day;
        this.scheduled_hour = scheduled_hour;
        this.scheduled_minute = scheduled_minute;
    }

    public int getIntervalYears()
    {
    ...
    protected GregorianCalendar getNextRunTime(GregorianCalendar schedule_start)
    {
        ...
        next_run.set(Calendar.MONTH, scheduled_month);
        ...
        if (!next_run.after(schedule_start))
            next_run.add(Calendar.YEAR, interval_years);
        else
            next_run.add(Calendar.YEAR, interval_years - 1);
        return next_run;
    }
    ...
    {
        return ", interval_years = " + String.valueOf(interval_years) +
            ", scheduled_month = " + String.valueOf(scheduled_month) +
